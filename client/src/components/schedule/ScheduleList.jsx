...
const ScheduleList = () => {
  ...
  const getSchedule = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:2402/allSchedules", {
        headers: {
  ...
  const deleteSchedule = async (schedule_id) => {
    try {
      ...
      await axios.delete(`http://localhost:2402/scheduleDelete/${schedule_id}`, config);
      getSchedule();
            ...
            <tr key={schedule.schedule_id}>
              ...
              <td>
                <CButton className="btn bg-info" style={{ fontWeight: 700 }}>
                  <Link
                    to={`/scheduleLists/edit/${schedule.schedule_id}`}
